...
var InputHelper = function () {
    ...
    this.assertToContain = function (key, testInstance, ignoreCase) {
        console.log("Inside Input Helper Assert to contain:- vys= " + testInstance.VariableName + " " + testInstance.VariableName.startsWith('#arraycontain'));
        if (key == "") {
            if (testInstance.VariableName.startsWith('#arraycontain')) {
                console.log("Inside Input Helper Assert to contain:- vyse=");
                browser.getCurrentUrl().then(function (urle) {
                    console.log("Inside Input Helper Assert to contain:- vysem");
                    var splitedVar = testInstance.VariableName.split('#');
                    var assertresult = customAssertHelper.arrayComparisionAssert(splitedVar[2], splitedVar[3], true, transformation);
                    if (assertresult) {
                        expect("array one contains in second array").toEqual("array one contains in second array");
