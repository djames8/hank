-- Function: procprocessschedulerdata(text, bigint)

-- DROP FUNCTION procprocessschedulerdata(text, bigint);

CREATE OR REPLACE FUNCTION procprocessschedulerdata(
    groupname text DEFAULT NULL::text,
    userid bigint DEFAULT 2)
  RETURNS void AS
$BODY$
...
BEGIN
		userid := case when userid <= 0 then 2 else userid end;
                ...
                CREATE TEMP TABLE temp1 (Id BigInt, StartDateTime timestamp, DoProcess boolean, DontUpdateLastTimeProcessedAt boolean, tmpId integer);
                ...
                LOOP
                                ...
                                if(repeatTaskFrequency >= '1' and repeatTaskDuration >= '1' and (frequencyType != '1' or (lastExecuted is null or startDateTime > lastExecuted))) Then
                                ...
                                insert into temp1 (Id, StartDateTime, DoProcess, DontUpdateLastTimeProcessedAt, tmpId) values(scheduleId, startDateTime, doProcess, dontUpdateLastTimeProcessedAt, tmpId);
                End loop;
...
--select current_timestamp
                insert into "TblSchedulerHistory" ("SchedulerId", "GroupName", "Status", "IsCancelled", "CreatedOn", "ModifiedOn", "IsDeleted","CreatedBy","ModifiedBy", "SettingsJson")
                select t1.Id, groupName || '-' || t1.Id, 0, case when length(t2."SettingsJson"::json->>'ExtraDataPostedByCaller') > 0 and lower(t2."SettingsJson"::json->>'ExtraDataPostedByCaller') not like '%complete%' then true else false end, now(), now(), 'f', userId, userId, t2."SettingsJson" from temp1 t1
...
-- Update scheduler LastTimeProcessedAt, will only get updated for real execution not for recurring
                ...
                "LastTimeProcessedAt" = cast(current_timestamp::date || ' ' || "StartDateTime"::time as timestamp), 
                "ModifiedOn" = current_timestamp,
                "ForceExecute"='f',
                "SettingsJson" = case when length("SettingsJson"::json->>'ExtraDataPostedByCaller') > 0 then replace("SettingsJson"::Text, ',"ExtraDataPostedByCaller":"' || replace(replace(replace("SettingsJson"::json->>'ExtraDataPostedByCaller', '"', '\"'), E'\n', '\n'), E'\r', '\r') || '"', '')::json else "SettingsJson" end
                Where "Id" in (select Id from temp1 where COALESCE(temp1.DontUpdateLastTimeProcessedAt, 'f') != 't');
END;
$BODY$
  LANGUAGE plpgsql VOLATILE
  COST 100;
ALTER FUNCTION procprocessschedulerdata(text, bigint)
  OWNER TO postgres;
